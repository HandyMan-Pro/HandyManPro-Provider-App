...
Map<String, String> buildHeaderTokensForMultipart() {
  final header = buildHeaderTokens();
  header.remove(HttpHeaders.contentTypeHeader);
  return header;
}

Map<String, String> buildHeaderTokensForLanguage(String languageCode) {
  Map<String, String> header = {};

  if (appStore.isLoggedIn) header.putIfAbsent(HttpHeaders.authorizationHeader, () => 'Bearer ${appStore.token}');
  header.putIfAbsent(HttpHeaders.contentTypeHeader, () => 'application/json; charset=utf-8');
  header.putIfAbsent(HttpHeaders.acceptHeader, () => 'application/json; charset=utf-8');
  header.putIfAbsent(CustomHeader.LanguageCode, () => languageCode);
  header.addAll(defaultHeaders());
...
Future<Response> buildHttpResponse(
  ...
  try {
    ...
    print('buildHttpResponse: Exception occurred: $e');
...
Future buildMultiPartResponse({
  ...
  try {
    ...
    if (files != null && files.isNotEmpty) {
      ...
      if (files.validate().length > 0) multiPartRequest.fields.putIfAbsent('0', () => files.validate().length.toString());
    }
