...
Future<BaseResponseModel> savePromotionalBannerPayment(Map request) async {
  var multiPartRequest = await getMultiPartRequest('banner-payment');
  multiPartRequest.headers.addAll(buildHeaderTokensForMultipart());
  for (final e in request.entries) {
    multiPartRequest.fields[e.key.toString()] = e.value?.toString() ?? '';
  }
  var response = await Response.fromStream(await multiPartRequest.send());
  return BaseResponseModel.fromJson(await handleResponse(response) as Map<String, dynamic>);
}
//endregion
